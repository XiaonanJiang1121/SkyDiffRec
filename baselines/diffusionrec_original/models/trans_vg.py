...
class TransVGSwin(nn.Module):
    def __init__(self, args):
        ...
        self.query = Parameter(torch.FloatTensor(args.batch_size, hidden_dim))
        self.reset_parameters()
        self.valid_learning = nn.Linear(hidden_dim * 2, hidden_dim)
    ...
    def prepare_diffusion_concat(self, device):
        """
        ...
        box_placeholder = torch.randn(self.num_proposals, 4,
    ...
    def box_feature_selction(self, bboxes, features, sentence_features, Threshold, next_boxes = None):
        ...
        v_l_scores = self.l2(self.Tanh(sentence_features_expand + features)).squeeze(2)
    ...
    def forward(self, img_data, text_data, word_selection, 
       ...
       # word_selection_expand = word_selection.unsqueeze(2).expand(text_src.size(0), text_src.size(1), text_src.size(2))
        ...
        text_src_sentence = self.text_proj(sentence_fea)
        #pdb.set_trace()
        ...
        if training_states != None:    
            x_boxes = self.prepare_diffusion_concat(device)
            x_boxes = x_boxes * images_whwh[:, None, :]
            #process_boxes = []
            #time_steps = 5
            ...
            for time, time_next in time_pairs:
                ...
                text_learn = self.valid_learning(torch.cat((text_src_sentence * alpha_next.sqrt(), c * self.query), 1))
                #text_src_sentence = text_src_sentence * alpha_next.sqrt() + c * self.query
                #sentence_fea = sentence_fea + self.query
                #pdb.set_trace()
                time_cond = torch.full((x_boxes.size(0),), time, device=device, dtype=torch.long)
                pred_bboxes, bboxes_features, scores = self.head(features_list, text_learn, x_boxes, time_cond, None)
                ...
                selected_boxes, selected_features, scores = self.box_feature_selction(pred_bboxes[pred_bboxes.size(0)-1],
                                                                               ...
                                                                               text_src_sentence, self.Threshold)
                '''             
            ...
            #pdb.set_trace()
            final_selected_features = bboxes_features[len(bboxes_features) - 1]
            vg_hs_trans = vg_hs.unsqueeze(1).expand(vg_hs.size(0), final_selected_features.size(1), vg_hs.size(1))
            cat_features = torch.cat((vg_hs_trans, final_selected_features), 2)
            prediction = self.bbox_embed(cat_features)
            ...
            #pdb.set_trace()
            return prediction.sigmoid(), scores[len(scores) - 1], sentence_fea, valid_sentence_fea
        else:
            #pdb.set_trace()
            final_features = []
            '''
            ...
            for time, time_next in time_pairs:
                ...
                c = (1 - alpha_next - sigma ** 2).sqrt()
                #noise = torch.randn_like(sentence_fea)
                text_src_sentence_expand = text_src_sentence.expand(self.query.size(0), text_src_sentence.size(1))
                text_learn = self.valid_learning(torch.cat((text_src_sentence_expand * alpha_next.sqrt(), c * self.query), 1))
                #pdb.set_trace()
                for i in range(0, text_learn.size(0)):
                    time_cond = torch.full((bs,), time, device=device, dtype=torch.long)
                    pred_bboxes, bboxes_features, scores = self.head(features_list, text_learn[i].unsqueeze(0), x_boxes, time_cond, None)
                    final_features.append(bboxes_features[len(bboxes_features) - 1][0])

                #pdb.set_trace()
                final_features = torch.stack(final_features)
                vg_hs_trans = vg_hs.unsqueeze(1).expand(final_features.size(0), final_features.size(1), vg_hs.size(1))
                cat_features = torch.cat((vg_hs_trans, final_features), 2)
                prediction = self.bbox_embed(cat_features)

                #torch.sum(self.query, dim = 0).unsqueeze(0) / self.query.size(0)
                #sentence_fea = sentence_fea * alpha_next.sqrt() + c * self.query[i] + sigma * noise
            ...
            #norm_pred_boxes = saved_boxes[len(saved_boxes) - 1] / images_whwh[:, None, :]
            #norm_pred_boxes = final_boxes / images_whwh[:, None, :]
            #pdb.set_trace()
