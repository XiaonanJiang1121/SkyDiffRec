...
class DynamicHead(nn.Module):
    ...
    def __init__(self):
        ...
        # Build RoI.
        pooler_resolution = 7
        pooler_scales = tuple([0.25, 0.125, 0.0625, 0.03125])
        sampling_ratio = 2
        pooler_type = 'ROIAlignV2'
        box_pooler = self._init_box_pooler(pooler_resolution, pooler_scales, sampling_ratio, pooler_type)
        ...
        activation = 'relu'
        num_heads = 6 #orig 6
        rcnn_head = RCNNHead(RCNN_hidden_dim, RCNN_dim_dynamic, RCNN_num_dynamic, pooler_resolution, 
                            d_model, dim_feedforward, nhead, dropout, activation)
        ...
        #boxes_selection
        self.Threshold = 0.7
        self.l = nn.Linear(RCNN_hidden_dim, 1)
        self.Tanh = nn.Tanh()
        # Init parameters.
    ...
    def box_feature_selction(self, bboxes, features, sentence_features, Threshold, next_boxes = None):
        ...
        v_l_scores = self.l(self.Tanh(sentence_features_expand + features)).squeeze(2)
    ...
    def forward(self, features, sentence_features, init_bboxes, t, init_features, RCNN_hidden_dim = 256, RCNN_dim_dynamic = 64):
        ...
        saved_features = []
        computed_scores = []
        saved_boxes.append(bboxes)
        saved_features.append(None)
        count = 0
        for head_idx, rcnn_head in enumerate(self.head_series):
            #pdb.set_trace()
            pred_bboxes, proposal_features = rcnn_head(features, saved_boxes[len(saved_boxes) - 1], 
                                                       saved_features[len(saved_features) - 1],
                                                       sentence_features, self.box_pooler, time,
                                                       RCNN_hidden_dim, RCNN_dim_dynamic)
            
            #pdb.set_trace()
            proposal_bboxes = pred_bboxes.detach()
            selected_boxes, selected_features, scores = self.box_feature_selction(proposal_bboxes, proposal_features, 
                                                                        sentence_features, self.Threshold, next_boxes = None)
            ...
            if self.return_intermediate:
                ...
                inter_pred_features.append(proposal_features)
                inter_pred_bboxes.append(pred_bboxes)
            '''

        '''
        if self.return_intermediate:
            return torch.stack(inter_pred_bboxes), torch.stack(inter_pred_features)
        '''
        if self.return_intermediate:
            return saved_boxes, saved_features, computed_scores
...
class RCNNHead(nn.Module):
    ...
    def __init__(self, RCNN_hidden_dim, RCNN_dim_dynamic, RCNN_num_dynamic, pooler_resolution, 
                 d_model, dim_feedforward=2048, nhead=8, dropout=0.1, activation="relu",
        ...
        self.inst_interact = DynamicConv(RCNN_hidden_dim, RCNN_dim_dynamic, RCNN_num_dynamic, pooler_resolution)
        ...
        self.vl_RCNN_fuse = nn.Linear(d_model * 2, d_model)
    ...
    def forward(self, features, bboxes, pro_features, vis_transformer_features,
                pooler, time_emb, RCNN_hidden_dim, RCNN_dim_dynamic):
        """
        ...
        pro_features2 = self.inst_interact(pro_features, roi_features, RCNN_hidden_dim, RCNN_dim_dynamic)
        ...
        #fuse transformer features
        region_features = fc_feature.view(N, nr_boxes, -1)
        vis_transformer_features_expand = vis_transformer_features.unsqueeze(1).expand(vis_transformer_features.size(0),
                                                                                       nr_boxes, vis_transformer_features.size(1))
        vis_fc_c = torch.cat([region_features, vis_transformer_features_expand], dim=2)
        #pdb.set_trace()
        vis_fc = self.vl_RCNN_fuse(vis_fc_c).reshape(N * nr_boxes, -1)
        ...
        return pred_bboxes.view(N, nr_boxes, -1), vis_fc.view(N, nr_boxes, -1)
...
class DynamicConv(nn.Module):
    ...
    def __init__(self, hidden_dim, dim_dynamic, num_dynamic, resolution):
        super().__init__()
    ...
    def forward(self, pro_features, roi_features, RCNN_hidden_dim, RCNN_dim_dynamic):
        '''
        ...
        param1 = parameters[:, :, :self.num_params].view(-1, RCNN_hidden_dim, RCNN_dim_dynamic)
        param2 = parameters[:, :, self.num_params:].view(-1, RCNN_dim_dynamic, RCNN_hidden_dim)
