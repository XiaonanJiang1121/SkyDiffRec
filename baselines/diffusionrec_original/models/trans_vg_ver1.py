...
class TransVGSwin(nn.Module):
    def __init__(self, args):
        ...
        self.query = Parameter(torch.FloatTensor(args.batch_size, self.textmodel.num_channels))
       ...
       # word_selection_expand = word_selection.unsqueeze(2).expand(text_src.size(0), text_src.size(1), text_src.size(2))
        ...
        if training_states != None:    
            ...
            for time, time_next in time_pairs:
                ...
                c = (1 - alpha_next - sigma ** 2).sqrt()
                noise = torch.randn_like(sentence_fea)
                #learned_tensor = torch.sum(self.query, dim = 0).unsqueeze(0) / self.query.size(0)
                print(sentence_fea.size(),alpha_next.sqrt(),self.query.size(),noise.size())
                sentence_fea = sentence_fea * alpha_next.sqrt() + c * self.query + sigma * noise
                #pdb.set_trace()
                #sentence_fea = sentence_fea * alpha_next.sqrt() + c * learned_tensor + sigma * noise
