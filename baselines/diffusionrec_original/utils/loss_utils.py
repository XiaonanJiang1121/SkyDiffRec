...
def trans_vg_loss_dif(batch_pred, batch_target):
    """Compute the losses related to the bounding boxes, 
       including the L1 regression loss and the GIoU loss
    """
    #pdb.set_trace()
    ...
    loss_b = 0
    loss_g = 0
    #batch_final = batch_pred
    #pdb.set_trace()
    batch_size = batch_pred.size(0)
    for i in range(0, batch_pred.size(1)):
        #num_boxes = 
        #num_boxes = 1
        loss_bbox = F.l1_loss(batch_pred[:,i,:], batch_target, reduction='none')
        #pdb.set_trace()
        loss_giou = 1 - torch.diag(generalized_box_iou(
            xywh2xyxy(batch_pred[:,i,:]),
            xywh2xyxy(batch_target)
        ))
        loss_b = loss_b + loss_bbox.sum() / batch_size
        loss_g = loss_g + loss_giou.sum() / batch_size
    


    losses['loss_bbox'] = loss_b / batch_pred.size(1)
    losses['loss_giou'] = loss_g / batch_pred.size(1)
...
def sentence_similarity(input1, input2):
    #pdb.set_trace()
    sentence1 = F.normalize(input1, p=2, dim=1)
    sentence2 = F.normalize(input2, p=2, dim=1)
    #loss = F.cosine_similarity(input1, input2, dim=1)
    loss = F.cosine_similarity(sentence1, sentence2, dim=1)
    return loss




# def sentence_region(inputted_sentences, reigion_features):
#     pdb.set_trace()
#     sentences = F.normalize(inputted_sentences, p=2, dim=1)
#     sentences_expand = sentences.unsqueeze(1).expand(sentences.size(0), reigion_features.size(1), sentences.size(1))
#     visual_emb_normalized = F.normalize(reigion_features, p=2, dim=2)
#     #sentence2 = F.normalize(input2, p=2, dim=1)
#     loss = F.cosine_similarity(sentences_expand, visual_emb_normalized, dim=2)
#     return loss
