...
The evaluation uses one unified prompt, runs one model at a time on a single
GPU, and records raw responses, parsed boxes, IoU, latency, and failure status in
resumable JSONL files. Missing or corrupt images are logged and skipped without
interrupting the split.
